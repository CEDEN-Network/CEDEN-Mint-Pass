...
describe.only("CedenMintPass: ", function () {
    ...
    it("mint() - try when not on free list or allow list during exclusiveWindow", async function () {
        await expect(ONFT_A.mint(1)).to.be.revertedWith("Allow List amount < mint amount")
    })
    ...
    it("mint() - when window opens up anyone can mint but must save for free minters", async function () {
        ...
        await expect(ONFT_A.connect(warlock).mint(10000)).to.be.revertedWith("Ceden: Mint exceeds supply")
